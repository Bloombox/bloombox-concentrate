...
all: build
	@echo "bloombox-concentrate is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-concentrate" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-concentrate"
endif
